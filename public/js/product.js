$(document).ready(function () {
    ...
    $(".select-color .color-inner").click(function () {
        ...
        if (!$(this).hasClass("active")) {
            ...
            $(".section-infomation .image-product>.image-gallery-" + key).css(
                "display",
                "block"
            );
        ...
        if (can_by == 0)
            $(".form-payment .row>div>button").prop("disabled", true);
        else $(".form-payment .row>div>button").prop("disabled", false);
        var qty = $(this).attr("data-qty");
        $("#qty").attr("max", qty);
    });
    $("button.add_to_cart").click(function () {
        var product_id = $(".color-product .color-inner.active").attr(
            "product-id"
        );
        ...
        $.ajax({
            ...
            success: function (data) {
                $("#cart-sidebar").remove();
                $(".mini-cart .top-cart-content").append(
                    htmlCart(data.response, data.url)
                );
                $(".support-cart .count_item_pr").empty();
                $(".support-cart .count_item_pr").append(
                    data.response.totalQty
                );
                Swal.fire({
                    title: "Thành Công",
                    text: data.msg,
...
function scrollToxx() {
    ...
    $(
        ".tab-header .nav-tab-custom>li.active, .tab-content>div.active"
    ).removeClass("active in");
    $(".tab-header .nav-tab-custom>li:nth-child(2)").addClass("active");
    $(".tab-header .nav-tab-custom>li:nth-child(2), #vote").addClass(
        "active in"
    );
}
function minusInput() {
    ...
    if (!isNaN(qty) & (qty > min)) result.value--;
    if (result.value == 0)
        $(".form-payment .row>div>button").prop("disabled", true);
    return false;
}
function plusInput() {
    ...
    if (!isNaN(qty) & (qty < max)) result.value++;
    if (result.value > 0)
